...
## list whose elements are matrices returned by the functions that we define within makeCacheMatrix function. 
## set_mat takes as input, the matrix from the enviornment from which it is called and returns it.
## get_mat returns the same matrix. The value it returns is stored by the
## list and used by the second function to evaluate the inverse of the input matrix.
